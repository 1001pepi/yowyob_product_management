...
function SearchBar({findInCategories, findInConditionnements, findInLangues, findInProducts, findInTaxes,setSpaceName, stringToSearch, setStringToSearch, categoriesResult,productsResult,setProductsResult, setCategoriesResult,updateProductsList,setUpdateProductsList, updateCategoriesList, setUpdateCategoriesList, productsList,setProductsList,categoriesList, setCategoriesList}){
    ...
    //fonction permettantt de récupérer la liste des produits//
    function getProducts(requestURL, tmpList, toSearch){
        //création de la requête
        var request = new XMLHttpRequest();
        ...
        request.onload = function(){
            ...
            if(requestStatus === 200){
                ...
                findInProducts(tmpList, toSearch)
                setProductsList(tmpList)

                if(next != null){
                    getProducts(next, tmpList, toSearch)
                }
            }
        }
    }
    ...
    //fonction pour filtrer les éléments dans une liste de produits
    function findInProducts(list, toSearch){
        const tmpList = list.filter(function(item, index, arr){
            if(item['name'].toUpperCase().includes(toSearch.toUpperCase())){
                return true
            }
    ...
    function handleResearch(toSearch){
        ...
        if(findInProducts){
            if(updateProductsList){
                getProducts(productsRequestURL, [], toSearch)
                setUpdateProductsList(false)

            }else{
                findInProducts(productsList, toSearch)
            }
